...
uint32_t ggapiGetStringOrdinal(const char * bytes, size_t len) noexcept {
    try {
        ...
        std::terminate(); // any string table put errors would be a critical error requiring termination
    }
}
...
bool ggapiStructPutString(uint32_t structHandle, uint32_t ord, const char * bytes, size_t len) noexcept {
...
bool ggapiListInsertString(uint32_t listHandle, int32_t idx, const char * bytes, size_t len) noexcept {
    return ggapi::trapErrorReturn<bool>([listHandle,idx,bytes,len]() {
        Global & global = Global::self();
        auto ss {global.environment.handleTable.getObject<ListModelBase>(ObjHandle{listHandle})};
        StructElement newElement { std::string(bytes, len)};
...
size_t ggapiStructGetString(uint32_t structHandle, uint32_t ord, char * buffer, size_t buflen) noexcept {
