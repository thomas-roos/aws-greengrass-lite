...
namespace data {
    ...
    public:
        struct CompEq {
            [[nodiscard]] bool operator()(const InternedString & a, const InternedString & b) const noexcept {
        ...
        ~InternedString() = default;
        InternedString(std::string_view sv) : _value{sv } {} // NOLINT(*-explicit-constructor)
        InternedString(std::string s) : _value{std::move(s)} {} // NOLINT(*-explicit-constructor)
    ...
    class StringTable {
        ...
        std::unordered_map<InternedString, StringOrd, InternedString::Hash, InternedString::CompEq> _interned;
        std::unordered_map<StringOrd, InternedString, StringOrd::Hash, StringOrd::CompEq> _reverse;
    public:
        StringOrd testAndGetOrd(std::string const & str) const {
        ...
        StringOrd getOrCreateOrd(std::string const & str) {
            ...
            if (ord.isNull()) {
                ...
                if (i == _interned.end()) {
                    ...
                    StringOrd new_ord {static_cast<uint32_t >(std::hash<std::string>()(str))}; // distribute IDs
