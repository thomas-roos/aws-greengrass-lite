    ...
    GIVEN("An instance of recipe reader") {
        ...
        WHEN("Reading a hello world recipe") {
            ...
            THEN("The recipe is read") {
                ...
                REQUIRE_THAT(recipe.configuration.defaultConfiguration->get("Message").getString(), Equals("world"));
                ...
                REQUIRE(recipe.manifests[0].lifecycle.find("run") != recipe.manifests[0].lifecycle.end());
                REQUIRE(recipe.manifests[0].lifecycle.at("run").isScalar());

                REQUIRE(recipe.manifests[0].lifecycle.at("run").getString() == "python3 -u {artifacts:path}/hello_world.py \"{configuration:/Message}\"\n");
                ...
                REQUIRE(recipe.manifests[1].lifecycle.find("run") != recipe.manifests[1].lifecycle.end());
                REQUIRE(recipe.manifests[1].lifecycle.at("run").isScalar());
                REQUIRE(recipe.manifests[1].lifecycle.at("run").getString() == "python3 -u {artifacts:path}/hello_world.py \"{configuration:/Message}\"\n");
                ...
                REQUIRE(recipe.manifests[2].lifecycle.find("run") != recipe.manifests[2].lifecycle.end());
                REQUIRE(recipe.manifests[2].lifecycle.at("run").isScalar());
                REQUIRE(recipe.manifests[2].lifecycle.at("run").getString() == "py -3 -u {artifacts:path}/hello_world.py \"{configuration:/Message}\"\n");
            }
        }
        WHEN("Reading a recipe with dependencies") {
            ...
            THEN("The recipe is read") {
                ...
                REQUIRE(recipe.manifests[0].lifecycle.find("run") != recipe.manifests[0].lifecycle.end());
                REQUIRE(recipe.manifests[0].lifecycle.at("run").isScalar());
                REQUIRE_THAT(recipe.manifests[0].lifecycle.at("install").getString(), Equals("echo Hello"));
                REQUIRE(recipe.manifests[0].lifecycle.find("install") != recipe.manifests[0].lifecycle.end());
                REQUIRE(recipe.manifests[0].lifecycle.at("install").isScalar());
                REQUIRE_THAT(recipe.manifests[0].lifecycle.at("run").getString(), Equals("echo World!"));
                ...
                REQUIRE(recipe.componentDependencies.find("aws.greengrass.tokenexchangeservice") != recipe.componentDependencies.end());
                REQUIRE(recipe.componentDependencies.find("aws.greengrass.s3service") != recipe.componentDependencies.end());
                REQUIRE_THAT(recipe.componentDependencies.at("aws.greengrass.tokenexchangeservice").versionRequirement, Equals("^2.0.0"));
                REQUIRE_THAT(recipe.componentDependencies.at("aws.greengrass.tokenexchangeservice").dependencyType, Equals("HARD"));
                REQUIRE_THAT(recipe.componentDependencies.at("aws.greengrass.s3service").versionRequirement, Equals("^3.0.0"));
                REQUIRE_THAT(recipe.componentDependencies.at("aws.greengrass.s3service").dependencyType, Equals("SOFT"));
                REQUIRE(recipe.configuration.defaultConfiguration->hasKey("Message"));
                REQUIRE_THAT(recipe.configuration.defaultConfiguration->get("Message").getString(), Equals("Hello World!"));
            }
        }
        WHEN("Reading a recipe with artifacts") {
            ...
            THEN("The recipe is read") {
                ...
                REQUIRE(recipe.getManifests()[0].artifacts[0].uri == "s3://mock-bucket/java/plugin.jar");
                REQUIRE(
                ...
                REQUIRE(
                    recipe.getManifests()[1].platform.nucleusType
                    == "lite");
                REQUIRE(
                    recipe.getManifests()[1].platform.architecture
                    == "aarch64");
                ...
                REQUIRE(
                    recipe.getManifests()[2].platform.nucleusType
                    == "lite");
                REQUIRE(
                    recipe.getManifests()[2].platform.architecture
                    == "amd64");
